...
//
	...
	def "Verify swatch selection when there are out of stock variants_15733"()
      ...
      //SPRING-15770 Dockers - DE - Verify that the Failed Search page does not display any products on the grid, when the user has placed an invalid search.
    @DEDockersSmoke
    def "Verify invalid search"(){
        when: "We are Home page & searching Invalid Product"
        to LevisHomePage
        at LevisHomePage
        searchInvalidProduct()

        then: "Check Failed search Page and no item found"
        at FailSearchResultPage
        verifyNoItemFound()
    }
    
    //SPRING-15764 Dockers - GB - Verify that "You Might Also Like" Product Recommendation section is displayed on the Failed Search page, when the user has placed an invalid search.
    @GBDockersSmoke
    def "Verify you might like  sec after invalid search"(){
        when: "We are Home page & searching Invalid Product"
        to LevisHomePage
        at LevisHomePage
        searchInvalidProduct()
