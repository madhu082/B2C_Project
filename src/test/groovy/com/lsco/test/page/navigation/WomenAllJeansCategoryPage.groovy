...
// added by Suprito after 12th March 2015
	...
	def verifyNumberOfProductsDisplayed()
	{
		Thread.sleep(7000)
		...
		Thread.sleep(7000)
		def prodSize = $("#container_results>li").size()
		println prodSize
	...
	def applyFiltersinWomanJeansPage()
	{
		Thread.sleep(5000)
	...
	def jeansredirectionToPDP(){
		
		def jeansName= $(".name").text().toUpperCase()
		println jeansName
		$("#container_results>li:nth-child(1)>div:nth-child(1)>a>img").click()
		Thread.sleep(5000)
		def jeansInPDP= $(".title").text().toUpperCase()
		println jeansInPDP
		assert jeansInPDP.contains(jeansName)
		return true
	}
