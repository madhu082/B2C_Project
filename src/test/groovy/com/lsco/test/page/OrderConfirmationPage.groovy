...
class OrderConfirmationPage extends Page {
	...
	def fillingRegInfoOrderCnfirmationPage() {
		...
		Thread.sleep(2000)
		//$("label[for='age-chkbox']").click()---Suprito after 31 st March 2015
	...
	def verifyErrorMessageForAlreadyRegsteredUsrFlow() {
		...
		Thread.sleep(2000)
		//$("label[for='age-chkbox']").click() -- Suprito 31st March 2015-----
		def actions=new Actions(driver);
		WebElement agechkbox=$(".checkbox-label.tag-label").firstElement()
		actions.moveToElement(agechkbox, 10, 2).click().build().perform()
	...
	def chksavingsAmount(String savingsAmount){
		String Savings=""
