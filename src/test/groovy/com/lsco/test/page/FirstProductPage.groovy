...
//				
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 ...
	 def verifyHoveringOverSwatchesOutOfStockVariants() {
		...
		Thread.sleep(2000)
		 ...
		 $(".size-swatch-wrapper.notouch-device.not-available.not-available-up>img").isDisplayed()
		 $(".size-swatch-wrapper.notouch-device.not-available.not-available-up>a").click()
		 
		 
			waitFor(60, 3){$(".color-swatches>li>img:nth-child(2)")}
			assert $(".color-swatches>li>img:nth-child(2)").hasClass("overlay")== true
 ...
 //		$(".zoom-viewer-wrap>img").click()
	...
	def selectAnySize() {
	 $("#pdp-buystack-size-values>li", 0, class: "size-swatch-wrapper notouch-device").click()
	 }
	//Added By Dipannita
	def SelectRandomSwatch() {
		int minimum =1
        int maximum =10

        int i = minimum + (int)(Math.random()*maximum)
		$(".color>ul>li:nth-child("+i+")>img").click()
		}
	
	//Added By Dipannita
	 def VerifySwatchSelected() {
		 $(".color>ul>li:nth-child(3)").hasClass("color-swatch notouch-device selected")
		return true
		}
	 ...
	 def doingAddToBagWithoutSelectingAttribute()
{
WebDriverWait wait5 = new WebDriverWait(driver, 30)
WebElement element = wait5.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a.button-large.btn-highlight.add-to-bag")))
// clicking on Add To Bag button
$("a.button-large.btn-highlight.add-to-bag").click()
Thread.sleep(3000)

// verifying the error message for adding a 1- D product without attribute selection
def errorScreenTxt =  $("#pdp-buystack-button-error>p").text().toUpperCase()
println errorScreenTxt
def addingWithoutAttr = PropertyProvider.getInstance().getLocalizedPropertyValue("addToBagWithoutAttribute").toUpperCase()
assert errorScreenTxt.contains(addingWithoutAttr)

//verifying that no items got added to bag
def zeroItemInBag = $(".mini-cart>h3>span").text()
println zeroItemInBag
def noItem = PropertyProvider.getInstance().getLocalizedPropertyValue("noItemInMiniCart")
println noItem
assert zeroItemInBag == noItem
...
def verifyHoveringOverSwatchesLatest() {
Thread.sleep(10000)
$("ul.color-swatches>li:nth-child(2)>img:nth-child(1)").click()
Thread.sleep(2000)
def skuNoFirstColour = $(".style-no").text()
println skuNoFirstColour
def priceFirstColour = $(".then-price").text()
...
// hovering to the second color
interact {
	moveToElement($("ul.color-swatches>li:nth-child(3)>img:nth-child(1)"))
}
	def skuNoSecondColour = $(".style-no").text()
	println skuNoSecondColour
	def priceSecoundColour = $(".then-price").text()
...
//---------------------Suprito after 31 st March 2015

 def verifyitemdontHaveItemLvlPromo()
 
 {   // below is the control for checking item level promotion.Boolean should return false
	 boolean bol8 = $(".item-promo.clearfix>div").isDisplayed()
